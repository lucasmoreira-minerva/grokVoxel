...
export function buildRenderPrompt(projectId: string): string {
  ...
  return [
    ...
    `Captions: enabled=${project.brief.captions?.enabled ?? true}, scale=${project.brief.captions?.scale ?? 0.62}, style=${project.brief.captions?.style ?? "clean"}, position=${project.brief.captions?.position ?? "bottom"}.`,
    `Approved storyboard / beats file: ${beatsPath}`,
    `Work under: ${outDir}`,
    `For each scene: if image_url/image_path already set, reuse that still; else generate keyframe from image_prompt (prefer image_edit with project assets as refs). Then image_to_video (~6s).`,
    `Then run deterministic assembly scripts from ${skill}/scripts (tts if needed, pick_music, assemble with caption_scale).`,
